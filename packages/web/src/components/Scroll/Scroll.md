...
La tinta es un **rol de borde del contrato mezclado con `color-mix`** —hoy `border.focus` al 40 %, en la constante `INK` de `Scroll.css.ts`— y el grosor sale de `vars.space.xl`. Los dos se cambian en un sitio y valen para todos los temas: medido sobre el render, el filo separa de la superficie 53 y 47 puntos de luminancia en `dark` y `light`.
