...
export function HeroRight(props: HeroSlotProps): ReactElement {
  const { children, className, ...style_rest } = props;
  const { className: sprinkle_class, style, rest } = ExtractStyleProps(style_rest);
  return (
    <Box className={cx(styles.slot, sprinkle_class, className)} style={style} {...rest}>
      {children}
    </Box>
  );
}

HeroRight.displayName = "Hero.Right";
