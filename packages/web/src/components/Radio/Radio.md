...
`styles.size` fija `radioSize` en `calc(vars.size.control.<size> / 2)`, igual que Checkbox y Switch, de
...
`radioColor` sigue inyectándose con `assignInlineVars` porque depende de la prop `color`; el tamaño no
