...
**W5.1 y W5.2 están cerradas y los seis paquetes están publicados en npm en `1.0.0`** desde el 2026-08-19 (`docs/release-checklist.md`). Publica el CI con `pnpm release`, que redacta las notas desde los commits. **Del gate de W5 solo falta la verificación de consumo** en la landing de Rosette (`prompts/2-web/W5.3-verificacion-en-rosette.md`). No se consiguió la procedencia npm: la exige sobre repositorio público y éste es privado — decisión pendiente, ver el checklist. **La 1.0.0 no dejó tags**: `changeset publish` debería crearlos y `git push --follow-tags` empujarlos, y no hay ninguno ni local ni en el remoto.

**En marcha está P5** del plan de performance (`docs/reviews/plan-performance-web-2026-08-14.md`): el tema fuera del camino de render (ADR-150). 59 componentes de raíz de servidor de 154. `Hero` y `Section` —los dos dueños del elemento que marca el LCP— **ya salieron del cliente** con una cáscara que recibe children, así que la decisión B está resuelta. **El gate de P0 no está cumplido** y no lo estará sin el entorno único de ADR-149. Los dos baselines visuales siguen derivando por separado (`win32` y `linux`), pero la publicación de la 1.0.0 pasó el gate visual en `linux`: `publish` depende de él, así que las 75 láminas casaron.

**Los dieciséis temas se llaman por su color** desde ADR-172 y su repertorio está cerrado: tres pares comparten primario a propósito —`marte`/`eclipse`, `cometa`/`vela`, `nova`/`quasar`— y se separan por el degradado. En paralelo sigue abierta **RP** (`prompts/2.4-rosette-product/`), la maqueta de Rosette, y **C1** (`prompts/6-consumidores/`) es el encargo que lleva una landing de producto a Nebula al 100%.
