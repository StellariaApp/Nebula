...
>
> **Revisión de budgets (2026-09-12, lote Rosette → Nebula — ADR-187 a ADR-198)**: entran `Dock` (patrón, medido 55,69 → tope **58 kB**; compone `Affix` + `GlassSurface` + `Popover` + `ActionIcon`), `Cutout` (primitivo ≤12), `VideoPlayer` y `AudioPlayer` (banda **media**, sin dependencias: son `<video>`/`<audio>` + `range`), `Viewer` (compuesto ≤48) y `MediaCard` (compuesto que monta `VideoPlayer`, tope 70). `AppShell` absorbe `AppShell.Scroll`, el contexto de desplazamiento y `Section hanging` dentro de su tope (26,21 de 27). Los números finales de cada entrada nueva se fijan en `.size-limit.js` al cerrar el lote.
