...
- **Contexto**: `docs/00-inventory.md` (fila 37 del anexo) reparte el `Segment` de fonicredito en dos componentes de Nebula: _"Control→SegmentedControl; Content swipeable→Tabs native"_. Al usarlo en la práctica ese reparto obliga al consumidor a cablear a mano el `value` entre `SegmentedControl` y `Tabs` en cada pantalla, que es exactamente el prop-drilling que el compound de fonicredito evita. Además, `Switch` y `SegmentedControl` animaban con transiciones CSS, sin el spring del tema que la plantilla canónica fija como capa 3 (ADR-004/ADR-018).
