...
- **Cambiar a un tema custom en caliente** ([ADR-121](adr/ADR-121-set-theme-acepta-un-tema-entero.md)): `setTheme` acepta un nombre oficial **o un `NebulaTheme` completo**, así que un tenant o un producto se retiñe entero sin remontar nada. Con un objeto se aplica por vars inline —la misma ruta que `defaultTheme`— y cambian las dos mitades a la vez: las vars CSS y la data no-CSS del contexto, que es lo que hace de `motion.tier` y `effects.glass.enabled` interruptores en caliente. **Un tema custom no se persiste**, porque no se puede reconstruir desde un nombre guardado; se persiste su `meta.scheme`, así que al recargar se cae al tema oficial del mismo scheme y `ColorSchemeScript` sigue pintando antes del primer frame. Quien quiera persistir el suyo guarda su propio selector y monta con el objeto en `defaultTheme`.
