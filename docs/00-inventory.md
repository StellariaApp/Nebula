...
| SimpleGrid                                  | prim   | WN   | 1    | P2·P3·ST(Grid.Simple)                       | core    | ST Grid.Simple. **ADR-201**: `SimpleGrid.Cell span rowSpan` para la celda que abarca columnas y filas, responsive como `cols`                                                                                                                                                                                                                                                                                                   |
...
| AppShell (+Header/Navbar/Aside/Footer/Main) | comp   | W    | 1    | P3·TFV(Navbar/Sidebar/Footer/Aside/Sidenav) | core    | Nuevo; absorbe el shell de TFV. **Los cinco nombres del paréntesis no son exports**: son props de slot (`header`, `navbar`, `aside`, `footer`) más `contentId` para el `<main>`. **ADR-204**: `useScrollSpy scroller` vigila la caja de `AppShell.Scroll`                                                                                                                                                                       |
...
| Stat                                    | comp   | WN   | 2    | P2·P3                        | core      | **ADR-200**: `uppercase={false}` deja el rótulo en caja normal y sin tracking                                                                                                                                                     |
...
| Componente                     | Clas.  | Plat                | Tier | Fuentes                         | Destino | Base / nota                                                                                                                                            |
| ------------------------------ | ------ | ------------------- | ---- | ------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Modal                          | comp   | WN                  | 1    | P2·P3·TFV                       | core    | Web: `<dialog>` + React Aria useDialog; TFV Modal (drawer/blurred/responsive) referencia de API. **ADR-203**: `initialFocus` — el foco cae en el panel |
...
| Dock                           | patrón | W                   | 2    | Rosette·Stellaria·Polaris·Iris  | core    | **ADR-193**: la caja flotante de preferencias (`Affix` + `GlassSurface` + `Popover`) que se recoge bajo `phone`; los controles son del producto        |
...
| VideoPlayer                                         | W    | 2    | Rosette                        | core    | **ADR-195**: reproductor **en línea** con mandos propios, sin dependencias, en `/media` y en el barrel (`MediaCard` lo monta)                   |
| AudioPlayer                                         | W    | 2    | Rosette                        | core    | **ADR-195**: nota de voz; comparte pista y volumen con `VideoPlayer` (`styles/player-controls.css.ts`); en `/media`                             |
| Viewer                                              | W    | 2    | Rosette                        | core    | **ADR-196**: visor **sin panel** — arrastre continuo, pellizco, rueda, tira `scroll-snap`. `Lightbox` sigue para el visor con panel             |
| MediaCard                                           | W    | 2    | Rosette                        | core    | **ADR-198**: la tarjeta de medio de todas las rejillas — láminas que pasan solas, clip, `playable`, esquinas que no se pisan, `href` o `onOpen` |
...
| Componente                                                                             | Plat | Tier | Fuentes                           | Destino | Nota                                                                                                                                                                                                                                                                                               |
| -------------------------------------------------------------------------------------- | ---- | ---- | --------------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Cutout                                                                                 | prim | W    | 2                                 | Rosette | core                                                                                                                                                                                                                                                                                               | **ADR-197**: fondo (ranura) + figura recortada colocada con vars; el visual de toda la landing de Rosette |
...
| NoiseTexture / NoiseOverlay                                                            | WN   | 3    | P2·P3                             | core    | **En web el export es `NoiseOverlay`**; `NoiseTexture` es la hoja reutilizable `styles/noise.css.ts` (`grain`, más `opacity` en `styles/noise.vars.css.ts`) que consumen GlassSurface, GradientBackground y MeshGradientBg                                                                         |
