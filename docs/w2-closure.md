...
6. **`aria-dialog-name`**: el `aria-labelledby` del Modal dependía del *slot id* de `useDialog`, que falla cuando el contenido solo se monta al abrir.
...
3. **Animación de salida y tests**. Las aserciones de desmontaje necesitan `waitFor`. Está documentado en `Modal.md`, pero conviene recogerlo en la plantilla canónica antes de W3.
4. **`Tabs` como envoltorio**. Sigue en el catálogo por estar en el inventario, pero es azúcar sobre `Segment`. Decidir si se mantiene.
5. **Revisión visual pendiente**. El gate axe y los budgets están verdes; la review contra las láminas `Foundations/Visual QA` de `docs/06-visual-language.md` no se ha hecho componente por componente.
