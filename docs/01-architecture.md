...
| Command             | propio sobre React Aria (`useComboBox`)      | —                                            | ADR-057 (cmdk descartado)                           |
...
| @internationalized/date                     | web                         | aritmética de fechas, calendarios y formato por locale que exigen `useCalendar`/`useDatePicker`/`useDateField`; ADR-050. **No aparece en la API pública**: el valor que cruza la frontera es un string ISO | `Date` nativo (off-by-one por zona en fechas sin hora), date-fns/Temporal polyfill (no es lo que consumen los hooks de Aria)           | ~14 kB brotli dentro de la cadena de fechas; ya era transitiva de react-aria |
...
| @vanilla-extract/*                          | web (build) · **themes (build + peer opcional)** | zero-runtime, theming por CSS vars. Desde [ADR-168](adr/ADR-168-el-contrato-css-se-muda-con-los-temas.md) `themes` declara el contrato, así que compila con el plugin (`devDependency`) y expone `@vanilla-extract/dynamic` como **peer opcional**: sólo lo usan los subpaths `/web`, y `native` —que depende de `themes`— no lo instala                                                                                                                                                                         | Panda CSS, StyleX (menos maduro para theme runtime dual)                                                                               | 0 runtime                                                                    |
...
| dnd-kit (línea estable 6.x)                 | web (subpath dnd)           | catálogo DnD/Kanban; es la línea con el patrón de teclado documentado (`KeyboardSensor` + `DragOverlay` + announcements) que exige 03 §1 — ADR-060                                                         | `@dnd-kit/react` 0.5.0 (API 0.x sin congelar, y W5 publica), pragmatic-dnd (obliga a escribir sortable + a11y a mano)                  | aislado en `/dnd`                                                            |
| embla                                       | web (subpath carousel)      | Carousel                                                                                                                                                                                                   | keen-slider                                                                                                                            | aislado en `/carousel`                                                       |
| react-player                                | web (subpath media)         | `Player`: vídeo de ~15 proveedores tras una sola API — ADR-060                                                                                                                                             | wrapper propio `<video>`+iframe (deuda recurrente de embeds), peer opcional (el subpath ya aísla; el peer solo añade un modo de fallo) | aislado en `/media`                                                          |
